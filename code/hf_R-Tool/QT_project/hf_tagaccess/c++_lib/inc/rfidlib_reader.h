...
extern "C" {
    ...
    err_t RFIDLIB_API RDR_EnableAsyncTagReportOutput(RFID_READER_HANDLE hr, BYTE type, WIN_MSG_t msg, HWND_PARENT hwnd, RFID_EVENT_CALLBACK cb);
	...
	*/
	...
	RFID_DN_HANDLE RFIDLIB_API LSG_CreateAlarmBookSet();
	err_t RFIDLIB_API LSG_AddOneAlarmBook(RFID_DN_HANDLE hBooks, BYTE bookNameLen, BYTE bookName[]);
	err_t RFIDLIB_API LSG_ShowAlarmBook(RFID_READER_HANDLE hr, RFID_DN_HANDLE hBooks,DWORD time);
	err_t RFIDLIB_API LSG_ShowAlarmBookV2(RFID_READER_HANDLE hr, BYTE opt,BYTE char_type,RFID_DN_HANDLE hBooks,DWORD time);
    ...
    err_t RFIDLIB_API RDR_EnableProtocolLog(RFID_READER_HANDLE hr, BYTE methType, WIN_MSG_t msg, HWND_PARENT hwnd, RFID_EVENT_CALLBACK_NEW cb, void *param);
    ...
    err_t RFIDLIB_API RDR_SetEventHandler(RFID_READER_HANDLE hr, BYTE eventType, BYTE methType, UINT msg, HWND_PARENT hwnd, RFID_EVENT_CALLBACK_NEW cb, VOID *param);
	...
	*/
	...
	err_t RFIDLIB_API RDR_GetVibrationState(RFID_READER_HANDLE hr, DWORD channel_cnt,BYTE* channel_ids,BYTE* item,DWORD *nSize);
	...
	err_t RFIDLIB_API LSG_ClearRecord(RFID_READER_HANDLE hr,BYTE flag,BYTE channelNo);
	err_t RFIDLIB_API LSG_InitChannelRecord(RFID_READER_HANDLE hr,BYTE mask, BYTE channel,DWORD inTotal,DWORD outTotal,DWORD inDay,DWORD outDay);
	err_t RFIDLIB_API LSG_GetChannelCnt(RFID_READER_HANDLE hr,BYTE num,BYTE *channelCnt);
	err_t RFIDLIB_API LSG_SetChannelCnt(RFID_READER_HANDLE hr,BYTE pwd[],BYTE num);
	err_t RFIDLIB_API LSG_CrossChannelPeople(RFID_READER_HANDLE hr,BYTE ch_n,BYTE dir,DWORD in,DWORD out);
    ...
    err_t RFIDLIB_API RDR_DeviceDebugLedOrOutput(RFID_READER_HANDLE hr, BYTE  time, BYTE frq, BOOL ledOrOutput);
	...
	err_t RFIDLIB_API LSG_FetchStoredDataAndAlaramFromGateExt(RFID_READER_HANDLE hr,BOOLEAN del, DWORD additional, DWORD additionalExt, BYTE gid,BYTE channelNo, DWORD * pcINTotal, DWORD * pcOUTTotal, DWORD * pcInDay, DWORD * pcOutDay);
	...
	err_t RFIDLIB_API LSG_ParseChannelRecord(RFID_DN_HANDLE hTagReport,BYTE* num,DWORD *pcInTotal, DWORD*pcOutTotal);
	...
	err_t  RFIDLIB_API  LSG_ParseRealtimePCRecordEx(RFID_DN_HANDLE hTagReport, BYTE *datetime, BYTE *dir, DWORD *PC,BYTE* sid,DWORD* pcChannel);
	...
	err_t RFIDLIB_API LSG_SetRadarSensitivity(RFID_READER_HANDLE hr,BYTE radarNo, BYTE sensitivity);
	err_t RFIDLIB_API LSG_GetRadarSensitivity(RFID_READER_HANDLE hr,BYTE radarNo, BYTE* sensitivity);
	...
	err_t RFIDLIB_API G310_ParseTagDataReportEx(RFID_READER_HANDLE hr, RFID_DN_HANDLE hTagReport, BYTE* uid_len, BYTE  uid[], BYTE* db_len, BYTE db_data[], BYTE time[],BYTE* direction, BYTE* channel);
	...
	err_t RFIDLIB_API Gate_SetVoicePrompts(RFID_READER_HANDLE hr,DWORD opt,LPCTSTR voiceText);
	err_t RFIDLIB_API Gate_GetVoicePrompts(RFID_READER_HANDLE hr,DWORD opt,LPTSTR voiceText,DWORD *nSize);
	...
	err_t RFIDLIB_API QBChannel_Init(RFID_READER_HANDLE hr,int exit_flag,int db_off,int db_cnt, BOOLEAN b_eas, BOOLEAN b_afi, BYTE afi_return, BYTE afi_borrow, BYTE rf_power,BYTE rf_level);
	err_t RFIDLIB_API QBChannel_GetData(RFID_READER_HANDLE hr, BYTE flash);
	err_t RFIDLIB_API QBChannel_ReadStart(RFID_READER_HANDLE hr,DWORD dir);
	err_t RFIDLIB_API QBChannel_ReadEnd(RFID_READER_HANDLE hr);
	err_t RFIDLIB_API BOOKCASE_GetIRStatus(RFID_READER_HANDLE hr,DWORD boardId,DWORD* ir_mask,DWORD* ir_status);


	err_t RFIDLIB_API RDR_SystemAuthenticate(RFID_READER_HANDLE hr,BYTE pwd[]);
	err_t RFIDLIB_API RDR_EnableSystemProtect(RFID_READER_HANDLE hr,DWORD func,BYTE pwd[]);
	err_t RFIDLIB_API RDR_DisableSystemProtect(RFID_READER_HANDLE hr);
